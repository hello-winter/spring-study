...
## 5.2.1 모 아니면 도
 ...
  java.lang.AssertionError;
  Expected: is <BASIC> 
       got: <SILVER>
 ```
 upgradeLevels() 메소드가 하나의 트랜잭션 안에서 동작하지 않았기 
...
```   
 ...
 - 비즈니스 로직 내의 트랜잭션 경계설정 =>
   UserService와 UserDao를 그대로 둔 채로 트랜잭션을 적용하려면 결국 트랜잭션의 경계설정 작업을 UserService 쪽으로 가져와야한다 
   UserService에는 트랜잭션 시작과 종료를 담당하는 최소한의 코드만 가져오게만들면 어느 정도 책임이 다른 코드로 분리둔채로 트랜잭션 문제를 
..리스트 5-38 ..   
